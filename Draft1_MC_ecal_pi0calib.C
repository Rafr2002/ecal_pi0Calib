...
// Best pair for MC: just choose the two clusters with the highest energies
void best_MC_pair(
...
//***NEED CALORIMETER DIMS
    ...
    TH1D *hBestDE = new TH1D("hBestDE", "Energy Difference between Photon Clusters",200,0,5);
...
//*Uncalibrated data(FIRST) event loop
    for (Long64_t i = 0; i < nEvents; i++) {
        ...
        best_MC_pair(ecal_e, nclus, best_icl, best_jcl, best_mass);
        ...
        // If we found a pair, compute dt from their cluster times
...
//** -------------------- Plotting --------------------
    ...
    h_pi0_mass->GetXaxis()->SetTitle("MC_G_{#pi^{0}} [GeV]");
    ...
    cDt->Divide(2,2);
    cDt->cd(1); gPad->SetLogy(); hBestDt->Draw();
    cDt->cd(2);
    hBestDx->Draw();
    cDt->cd(3); 
    // gPad->SetLogy(); 
