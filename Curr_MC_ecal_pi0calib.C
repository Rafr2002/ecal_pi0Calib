...
void find_N_highest_energies(const Double_t ecal_e[], int nclus, int N, std::vector<int>& iHigh, std::vector<double>& eHigh)
 {

    if (nclus <= 0 || N <= 0) { iHigh.clear(); eHigh.clear(); return; }


     // Collect (energy, index) pairs
    std::vector<std::pair<double,int>> ei_pairs;
    for (int icl = 0; icl < nclus; ++icl) {
        ei_pairs.emplace_back(ecal_e[icl], icl);
    }

    // Adjust ei_pairs size to number of valid signals
    N = std::min(N, static_cast<int>(ei_pairs.size()));

    // Sort ei_pairs to the front in descending energy
    std::sort(ei_pairs.begin(), ei_pairs.end(),
    [](const auto& a, const auto& b)
    { return a.first > b.first; });

    // Fill outputs
    iHigh.resize(N); eHigh.resize(N);
    for (int n = 0; n < N; ++n) {
        eHigh[n] = ei_pairs[n].first;
        iHigh[n] = ei_pairs[n].second;
    }
}

// Returns just the indices of top-N clusters by energy (N is capped at nclus)
static inline void getTopNByEnergy(const Double_t* ecal_e, int nclus, int N, std::vector<int>& iHigh) {

    iHigh.resize(nclus);    
    for (int i=0; i<nclus; i++){iHigh[i]=i;};

    if (N > nclus) N = nclus;
    std::nth_element(iHigh.begin(), iHigh.begin()+N, iHigh.end(),
        [&](int a, int b){ return ecal_e[a] > ecal_e[b]; });
    
    iHigh.resize(N);

    // Order the top N descending by energy
    std::sort(iHigh.begin(), iHigh.end(),
        [&](int a, int b){ return ecal_e[a] > ecal_e[b]; });
}


struct PairCand {//candidate cluster pairs
    int i, j;         // cluster indices
    double M;         // invariant mass
    double dt;        // time difference
    double dMass;     // smaller is better (|M - mπ0| or χ²)
};


void buildTopNPairs(
    std::vector<int>& iHigh, //indices of N highest energies
    Double_t* ecal_e, Double_t* clus_nblk,
    Double_t* ecal_x, Double_t* ecal_y,
    Double_t* clus_a_time,
    std::vector<PairCand>& cands)
{
    cands.clear();

    for (int a = 0; a < (int)iHigh.size(); ++a) {
        int i = iHigh[a];
        for (int b = a + 1; b < (int)iHigh.size(); ++b) {
            int j = iHigh[b];

            //initial cuts
            if (ecal_e[i] < 0.2 || ecal_e[j] < 0.2) continue;
            if ((int)clus_nblk[i] < 2 || (int)clus_nblk[j] < 2) continue;
            double dx = ecal_x[i]-ecal_x[j], dy = ecal_y[i]-ecal_y[j];
            if (std::sqrt(dx*dx+dy*dy) < 0.3) continue;
            if (clus_a_time[i] < 80 || clus_a_time[i] > 120) continue;
            if (clus_a_time[j] < 80 || clus_a_time[j] > 120) continue;

            double E1 = ecal_e[i], E2 = ecal_e[j];

            // if difference in hit time is greater than 50 ns(10 ns buffer) then skip
            double dt = clus_a_time[i] - clus_a_time[j];
            if (std::fabs(dt) > 50) continue;

            // Find mass using z_calo & vertex
            TVector3 dir1 = (TVector3(ecal_x[i], ecal_y[i], z_calo) - vertex).Unit();
            TVector3 dir2 = (TVector3(ecal_x[j], ecal_y[j], z_calo) - vertex).Unit();
            ...
            // closeness to mass of Pi0 + a small timing penalty
            double dMass = std::fabs(M - pi0_mass_pdg); // tweak 0.02 if you like

            cands.push_back({i, j, M, dt, dMass});
        }
    }
...
// NBest pair of clusters within the time window
void Nbest_pair_cuts(
    const Double_t ecal_e[], const Double_t clus_nblk[],
    const Double_t ecal_x[], const Double_t ecal_y[], 
    const Double_t clus_a_time[], 
    int nclus, int &best_icl, int &best_jcl, double &best_dt) {
...
// Best pair out of N clusters closest to pi0 mass within the time window
void best_pair_minMass_topN(
    const Double_t ecal_e[], const Double_t clus_nblk[],
    const Double_t ecal_x[], const Double_t ecal_y[], 
    const Double_t clus_a_time[], 
    int nclus, int nMax,
    int &best_icl, int &best_jcl, double &best_dt, double &best_mass){
    ...
    for (size_t i = 0; i < cand.size(); ++i){
        ...
        for (size_t j = i+1; j < cand.size(); ++j){
           ...
           Double_t deltaR = sqrt(pow(ecal_x[icl] - ecal_x[jcl], 2) + pow(ecal_y[icl] - ecal_y[jcl], 2));
            ...
            double M  = (ph1 + ph2).M();
            double dMass = fabs(M-pi0_mass_pdg);
            

            if (dMass < best_dMass){
                best_dMass=dMass;
                best_icl  = icl; best_jcl  = jcl;
                best_dt   = dt; best_mass = M;
            }
        }
    }
}
    ...
    for (size_t i = 0; i < cand.size(); ++i){
        ...
        for (size_t j = i+1; j < cand.size(); ++j){
           ...
           Double_t deltaR = sqrt(pow(ecal_x[icl] - ecal_x[jcl], 2) + pow(ecal_y[icl] - ecal_y[jcl], 2));
            ...
            double M  = (ph1 + ph2).M();
            double theta_exp = TMath::ACos(1-(pi0_mass_pdg*pi0_mass_pdg) / (2*sqrt(E1*E2)))* (180.0 / TMath::Pi());
...
// Best pair tuned for MC observables
void best_MC_pair(
    const Double_t ecal_e[], const Double_t clus_nblk[],
    const Double_t ecal_x[], const Double_t ecal_y[],
    const Double_t clus_a_time[],
    int nclus, int &best_icl, int &best_jcl, double &best_dt)
{
    ...
    for (int icl = 0; icl < nclus; ++icl) {
        for (int jcl = icl + 1; jcl < nclus; ++jcl) {
            ...
            // Energy cuts
            if ((ecal_e[icl] + ecal_e[jcl]) < 0.5) continue;  // sum > 0.5 GeV
            if (ecal_e[icl] < 0.2 || ecal_e[jcl] < 0.2) continue; // each > 0.2 GeV

            // Minimum of 2 blocks per cluster
            if (clus_nblk[icl] < 2 || clus_nblk[jcl] < 2) continue;

            // Pair Hit Seperation
            Double_t deltaR = sqrt(pow(ecal_x[icl] - ecal_x[jcl], 2) +
                                   pow(ecal_y[icl] - ecal_y[jcl], 2));
            if (deltaR < 0.3) continue;

            //dT tuned for MC cluster timing spectrum (50–90 ns)
            if (clus_a_time[icl] < 50 || clus_a_time[icl] > 90) continue;
            if (clus_a_time[jcl] < 50 || clus_a_time[jcl] > 90) continue;

            // Maximum time difference of 40 ns
            double dt = clus_a_time[icl] - clus_a_time[jcl];
            if (fabs(dt) < fabs(best_dt) && fabs(dt) <= 50) {
                best_dt  = dt;
                best_icl = icl;
...
// Light cut Best pair for MC: just choose the two clusters with the highest energies
void LIGHTbest_MC_pair(
    const Double_t ecal_e[], 
    int nclus, 
    int &best_icl, int &best_jcl, 
    double &best_mass)
{
    best_icl = best_jcl = -1;
    best_mass = -1.0;
...
void MC_ecal_pi0calib() {
	...
	ch->Add("/volatile/halla/sbs/rfruiz/replayed_Digd_10K_pi0_Ecal_cal.root");

    // /volatile/halla/sbs/rfruiz/replayed_Digd_10K_pi0_Ecal_cal.root
    // /volatile/halla/sbs/jonesdc/output/reduced/rootfiles/gep5_fullreplay_nogems_5056_.root

    // Disable all branches initially for efficiency
    ch->SetBranchStatus("*", 0);  // disable all
    //re-enable ECal branches
    ch->SetBranchStatus("earm.ecal.*", 1);
    ch->SetBranchStatus("Ndata.earm.ecal.*", 1);
 

    // Containers for energy, x, y of each cluster
    ...
    TH1F *h_pi0_mass = new TH1F("h_pi0_mass", "MC Pi0 Invariant Mass;M_{#pi^{0}} [GeV];Events", 80, 0, 0.6);
    TH1F *h_pi0_mass_corr = new TH1F("h_pi0_mass_reco", "Reconstructed MC #pi^{0} Invariant Mass;M_{#pi^{0}} [GeV];Events", 80, 0, 0.6);

	TH1F *hClusTime = new TH1F("hClusTime",
    "MC Photon Cluster Arrival Times; Time [ns];Counts", 200, 0, 200);

    
    TH1F *h_Epi0      = new TH1F("h_Epi0",
    "MC Pi^{0} Energy;E_{#pi^{0}} [GeV];Events", 140, 0, 14);
    TH1F *h_Epi0_corr = new TH1F("h_Epi0_corr",
    "Reconstructed MC Pi^{0} Energy;E_{#pi^{0}} [GeV];Events", 140, 0, 14);
    ...
    for (Long64_t i = 0; i < 20; i++) {
        ...
        cout << "Event " << i 
            << " clusters: " << nclus
            << ", good blocks: " << ngoodblk 
            // << ", good block ids: " << goodblock_id.size() << endl;
            <<endl;
    ...
    for (Long64_t i = 0; i < nEvents/100; ++i) {
        ...
        for (int b = 0; b < ngoodblk; ++b) {
            ...
            cout << "current block id : " << bid << endl;
            cout << "current row id : " << row << endl;
            cout << "current col id : " << col << endl;
            if (bid >= 0) {
                blockIDs.insert(bid);
    ...
    TH1D *hBestDt = new TH1D("hBestDt", "Time Difference between MC Photon Clusters",500,-50,50);
    TH1D *hBestDE = new TH1D("hBestDE", "Energy Difference between MC Photon Clusters",200,0,5);
    TH2D *hEvsE = new TH2D("hEvsE", "Photon 2 Energy_{Cluster} vs MC Photon 1 E_{Cluster}",200,0,7,200,0,7);
    TH1D *hBestDx = new TH1D("hBestDx", "Distance between MC Photon Clusters",50,0,5);
    TH1D *hAngle = new TH1D("hAngle", "Angle between MC Photon Clusters",100,0,30);
    TH1D *hAngleCut = new TH1D("hAngleCut", "Angle between MC Photon Clusters After Cut",200,-30,30);
...
//*Uncalibrated data(FIRST) event loop
    for (Long64_t i = 0; i < nEvents; i++) {
        ...
        double best_dt = 1e9;
        // New MC pairing func (time window 50–90 ns)
        best_MC_pair(ecal_e, clus_nblk, ecal_x, ecal_y, clus_a_time, nclus, best_icl, best_jcl, best_dt);
        

        // If pair, fill histograms
	...
	if(fabs(best_dt) > 4) continue;
        ...
        // std::vector<double> eTop; //Top N energies
        // std::vector<int> iTop; //indices of top N energies 
        // find_N_highest_energies(ecal_e, nclus, 4, iTop, eTop);
        ...
        if (best_icl >= 0 && best_jcl >= 0) {
            ...
            hMassVsX->Fill(ecal_x[best_icl], pi0_mass);
            hMassVsX->Fill(ecal_x[best_jcl], pi0_mass);

            hMassVsY->Fill(ecal_y[best_icl], pi0_mass);
            hMassVsY->Fill(ecal_y[best_jcl], pi0_mass);
...
//**Calibrated data(SECOND) event loop
    for (Long64_t i = 0; i < nEvents; i++) {
        ch->GetEntry(i);
        if (i % 10000 == 0) {
            cout << "Processing event (correction) " << i << " / " << nEvents << "\r";
...
//**(THIRD loop) Filling histos with Calibrated data
    for (Long64_t i = 0; i < nEvents; i++) {
        ch->GetEntry(i);
        ...
        if (best_icl >= 0 && best_jcl >= 0) {
            ...
            // Fill CORRECTED invariant mass vs position for each photon
            hMassVsX_corr->Fill(ecal_x[best_icl], pi0_mass_corr);
            hMassVsX_corr->Fill(ecal_x[best_jcl], pi0_mass_corr);

            hMassVsY_corr->Fill(ecal_y[best_icl], pi0_mass_corr);
            hMassVsY_corr->Fill(ecal_y[best_jcl], pi0_mass_corr);
...
//** -------------------- Plotting --------------------
    TCanvas *c = new TCanvas("c","Reconstructed MC #pi^{0} Invariant Mass Before and After Calibration", 800,600);
    ...
    leg->Draw();
    c->SaveAs("/work/halla/sbs/rfruiz/ecal_pi0Calib/plots/MC/top2E_MC_ecal_pi0_mass_calib_noiter.png");


    // Create a 2D histogram to visualize calibration coefficients in the detector geometry
    TH2F *h_coeff_map = new TH2F("h_coeff_map", "MC ECAL Block Calibration Coefficients;Column;Row",
        ncol, minCol, maxCol + 1l,
        nlin, minRow, maxRow + 1);
    TGraph *gr = new TGraph();
    TH1D *hCoeff = new TH1D("hCoeff","MC Calibration Coefficient Distribution",100,0,10);
    ...
    TCanvas *c_map = new TCanvas("c_map", "MC ECAL Coefficients Heatmap", 1600, 900);
    c_map->Divide(2,2);
    c_map->cd(1);
    h_coeff_map->SetStats(0);
    h_coeff_map->Draw("COLZ");
    c_map->cd(2);
    gr->SetTitle("MC Calibration Coefficients vs. Block ID");
    gr->SetMarkerStyle(6);
    gr->Draw("ap");
    ...
    c_map->cd(4);
    hEvsE->Draw("colz");
    c_map->SaveAs("/work/halla/sbs/rfruiz/ecal_pi0Calib/plots/MC/top2E_MC_ecal_coefficients_heatmap_noiter.png");
    ...
    // gPad->SetLogy(); 
    hBestDE->Draw();
    cDt->SaveAs("/work/halla/sbs/rfruiz/ecal_pi0Calib/plots/MC/top2E_MC_randomPlots.png");
    ...
    h_Epi0->Draw();
    h_Epi0_corr->Draw("SAME");        // draw after-calib first
    ...
    cE->SaveAs("/work/halla/sbs/rfruiz/ecal_pi0Calib/plots/MC/top2E_MC_ecal_pi0_energy.png");
